...
- Landing page: `src/components/Landing/` is the interactive portrait page. The face sits centered and is the main character; the right side holds draggable icon portals for Blog, Resume, and Contact. Dragging an icon into the mouth triggers the open-mouth state, then the mouth closes and the face shrinks while a route-colored circle expands to cover the screen before navigating. Keep the cartoon, personality-first composition and avoid turning it into a generic career landing page. Face images remain in `public/images/*.png` and filenames should stay stable. Run `python process_images.py` to process new face images from `images/`.
...
- Résumé: layout in `Resume.tsx` / `Resume.css`; content only in `Resume.data.ts` (exports `skills` object and a unified `timelineData` array for work, education, and awards). Each item may include an optional `description` string array for future detail rendering, and the final `timelineData` entry is intentionally kept as an empty placeholder for layout spacing. Update that file when text changes (e.g. from `resume_june2025.txt`); stagger animations rely on inline `--i` values.
